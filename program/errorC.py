...
#######################LL############################################
data_LL = np.loadtxt("err_cBL_LL.txt")
eLL=[]
vLL=[]
PLL=[]
eLL=data_LL[:,0]
...
#p_LL = np.reshape(PLL,(col_LL,col_LL))
###############################LH####################################
data_LH = np.loadtxt("err_cBL_LH.txt")
eLH=[]
vLH=[]
PLH=[]
eLH=data_LH[:,0]
...
p_LH = np.reshape(PLH,(col_LH,col_LH))
###############################HT#####################################
data_HT = np.loadtxt("err_cBL_HT.txt")
eHT=[]
vHT=[]
PHT=[]
eHT=data_HT[:,0]
...
########################R###########################################
data_R = np.loadtxt("err_cBL_R.txt")
eR=[]
vR=[]
PR=[]
eR=data_R[:,0]
...
plt.clim(1e-7,1e-1)
plt.pcolormesh(v_R, e_R/1e3, p_R, cmap='jet',shading='gouraud',norm=colors.LogNorm(vmin=p_R.min(),vmax=p_R.max()))
plt.clim(1e-7,1e-1)
plt.pcolormesh(v_LH, e_LH/1e3, p_LH, cmap='jet',shading='gouraud',norm=colors.LogNorm(vmin=p_LH.min(),vmax=p_LH.max()))
plt.clim(1e-7,1e-1)
