...
REAL(8) :: vis_out, T_in, conduc_out
!
!
!
!
...
!T_in = T_cr
!v_in = 1.0 / rho_cr!400.0!300.0
!flag = 1

!CALL CO2visco(vis_out, v_in, T_in,1.0_pr, 1)
!CALL CO2conduc(conduc_out, v_in, T_in, 1.0_pr, 1)

!print*, 'viscosity', vis_out
!print*, 'conductivity', conduc_out
...
!
!
OPEN (UNIT = 41,             FILE = 'transprop_LL.txt', &
...
&                        x_out_LL(k,p), a_out, res, y_test_LL(k), v_test_LL(k,p),flag)

       CALL CO2visco(vis_out, v_test_LL(k,p), T_BL_LL(k,p),1.0_pr, 1)
       CALL CO2conduc(conduc_out, v_test_LL(k,p), T_BL_LL(k,p), 1.0_pr, 1)
!
!
         WRITE(41,*) y_test_LL(k), v_test_LL(k,p), vis_out, conduc_out
!               
!
   ENDDO
...
!
!
OPEN (UNIT = 41,             FILE = 'transprop_LH.txt', &
&     FORM = 'formatted',  ACTION = 'write',         &
&   STATUS = 'replace')
!
!
...
&                          x_out_LH(k,p), a_out, res, y_test_LH(k), v_test_LH(k,p),flag)
!
!
       CALL CO2visco(vis_out, v_test_LH(k,p), T_BL_LH(k,p),1.0_pr, 1)
       CALL CO2conduc(conduc_out, v_test_LH(k,p), T_BL_LH(k,p), 1.0_pr, 1)
         WRITE(41,*) y_test_LH(k), v_test_LH(k,p), vis_out, conduc_out
!               
!
   ENDDO
ENDDO   !For all the tested values NNN_test x MMM_test in the Region
...
!
!
OPEN (UNIT = 41,             FILE = 'transprop_R.txt', &
&     FORM = 'formatted',  ACTION = 'write',         &
&   STATUS = 'replace')
!
!
...
&                          x_out_R(k,p), a_out, res, y_test_R(k), v_test_R(k,p),flag)

!
        CALL CO2visco(vis_out, v_test_R(k,p), T_BL_R(k,p),1.0_pr, 1)
        CALL CO2conduc(conduc_out, v_test_R(k,p), T_BL_R(k,p), 1.0_pr, 1)
        WRITE(41,*) y_test_R(k), v_test_R(k,p), vis_out, conduc_out
!               
!
   ENDDO
ENDDO   !For all the tested values NNN_test x MMM_test in the Region
...
!
!
OPEN (UNIT = 41,             FILE = 'transprop_HT.txt', &
&     FORM = 'formatted',  ACTION = 'write',         &
&   STATUS = 'replace')
!
!
...
&                       x_out_HT(k,p), a_out, res, y_test_HT(k), v_test_HT(k,p),flag)

     CALL CO2visco(vis_out, v_test_HT(k,p), T_BL_HT(k,p),1.0_pr, 1)
     CALL CO2conduc(conduc_out, v_test_HT(k,p), T_BL_HT(k,p), 1.0_pr, 1)
!
         WRITE(41,*) y_test_HT(k), v_test_HT(k,p), vis_out, conduc_out
!               
!
   ENDDO
ENDDO   !For all the tested values NNN_test x MMM_test in the Region
...
!print*, i,j,"v_in",v_in,"u_in",u_in, p_guess
        ...
        CALL CO2BLLT_EQUI(press, temp, sound, x_out, a_out, res, u_in,v_in,flag)
        p_guess = press
